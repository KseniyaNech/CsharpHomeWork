...
{
    List<int> list = new List<int>();//создается чистый лист для записи встречающихся чисел. 
                                   //пустые скобки означают, что лист без размера

    
    foreach (int item in arr) // команда foreach проходит по всему двумерному массиву всесто двух циклов for 
    {
        if (!list.Contains(item))//если в листе не содержится...
        {         
            int count = 0;   
        ...
        list.Add(item);//в лист добавляется искомое число.
    }
}
